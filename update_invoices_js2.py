...
'''
    ...
    # we want to replace from `let currentInvoices = [];` down to the end of the file except `}`
    # Wait, in the files, the last line is `}` 
    start_idx = content.find('let currentInvoices = [];')
    if start_idx == -1:
        start_idx = content.find('let allInvoices = [];')
        if start_idx == -1:
            print("Could not find start point in", filepath)
            return

    head = content[:start_idx]
    # In head, remove old unused variables
    head = head.replace('const btnPrev = document.getElementById("btn-prev-page");', '')
    head = head.replace('const btnNext = document.getElementById("btn-next-page");', '')
    head = head.replace('const pageInfo = document.getElementById("page-info");', '')
    head = head.replace("const btnPrev = document.getElementById('btn-prev-page');", '')
    head = head.replace("const btnNext = document.getElementById('btn-next-page');", '')
    head = head.replace("const pageInfo = document.getElementById('page-info');", '')
    head = head.replace('const btnSearch = document.getElementById("btn-search");', '') # we redefine it

    # fix openModal to search in allInvoices instead of currentInvoices
    head = head.replace('currentInvoices.find', 'allInvoices.find')

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(head + new_logic)
